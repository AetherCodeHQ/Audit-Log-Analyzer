...
**Audit Log Analyzer** is a security tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Audit-Log-Analyzer --path ./target

# With options
./Audit-Log-Analyzer --path ./target --format json --output report.json

# Verbose mode
./Audit-Log-Analyzer --path ./target --verbose
```
...
### Basic scan
```bash
./Audit-Log-Analyzer --path ./my-project
```

### JSON report
```bash
./Audit-Log-Analyzer --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Audit Log Analyzer
  run: ./Audit-Log-Analyzer --path . --format json --output report.json
```
